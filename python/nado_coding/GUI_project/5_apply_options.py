...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", \
        ...
        initialdir=r"C:\Users\hwonssam\workspace") #최초의 C:/경로를 보여줌
    #사용자가 선택한 파일 목록
    for file in files:
 ...
 """
    ...
    ######################################
        ...
        for idx, img in enumerate(images):
            #width가 원본아닐땐 이미지 크기 조정
            if img_width >-1 :
                img = img.resize(image_sizes[idx])
            result_img.paste(img, (0,y_offset))
            y_offset += img.size[1] +img_space #height 값만큼 더해줌 +사용자가 지정한 간격
        ...
        msgbox.showinfo("알림", "작업이 종료되었습니다.")
        #시작
