...
def rps():
    ...
    while True:
        try:
            ...
            if user_rps == "가위" or user_rps == "바위" or user_rps == "보" \
                or user_rps == '0' or user_rps == '1' or user_rps == '2' :
                break
            print("다시 입력해주세요")
        except ValueError:
            pass
    ...
    elif  user_rps == '0' or user_rps == '1' or user_rps == '2' :
        user_num = int(user_rps)+ 1
        user_rps = key_list[int(user_rps)]
    #나눗셈을 통해 승,패,무승부 3개의 조건문 설정
    #global 미사용시 에러 UnboundLocalError: local variable 'draw' referenced before assignment
    global draw, user_win, com_win
